...
Бот использует два секрета: **токен Telegram** (обязательно, от [@BotFather](https://t.me/BotFather))
и опциональный **NVD API-ключ**. Они хранятся только локально в файле `.env`, который:

- добавлен в `.gitignore` и **никогда не попадает в репозиторий** — в git лежит лишь
  `.env.example` с пустыми значениями как шаблон;
- внутри Docker передаётся через `env_file` и не зашивается в образ.

Чтобы токен не утекал в логи, логирование `httpx` приглушено до `WARNING`
(иначе библиотека печатала бы полный URL запроса вместе с токеном).

> **Если форкаете/клонируете проект:** заведите собственного бота у @BotFather и
> впишите свой токен в `.env`. Чужой токен в коде вы не найдёте — его там нет by design.
